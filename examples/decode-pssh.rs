...
fn main() -> Result<()> {
    ...
    if matches.get_flag("parse-widevine-data") {
        let buf = BASE64_STANDARD.decode(data).context("decoding base64")?;
        let pssh_data =
            WidevinePsshData::decode(Cursor::new(buf)).context("parsing Widevine PSSH data")?;
    ...
    if matches.get_flag("parse-playready-data") {
        let buf = BASE64_STANDARD.decode(data).context("decoding base64")?;
        let pssh_data =
            pssh_box::playready::parse_pssh_data(&buf).context("parsing PlayReady PSSH data")?;
        println!("PlayReady PSSH data: {pssh_data:?}");
        return Ok(());
    }
    let boxes = if matches.get_flag("hex") {
        from_hex(data).context("parsing the PSSH as hex")?
    } else {
        from_base64(data).context("parsing the PSSH as base64")?
    };
    if boxes.len() > 1 {
